...
/**
 * Classe utilizada para rodar a versão costumizada do Auto-WEKA 3.8.<br>
 * 
 * @author Antonino Feitosa e João Carlos
 * @version 1.2
 *
 */
...
public class RunAutoWeka01 {
	...
	private static int timeLimit = 60; 							// in minutes
	...
	private static String[] base_name = { "resources/Image.arff" };
	...
	private static void run(File output, String base, int seed, int mTime, int mem, int nBest, int pRuns) throws FileNotFoundException {
		...
		try {
			...
			for (int fold = 0; fold < numFoldsEvaluate; fold++) {
				...
				System.out.println("Number of Internal Runs (cv's): " + numberInternalRuns + " - Error => " + 
				mainError + "Metricas => " + Utils.arrayToString(bestConfiguration.getOptions()).replaceAll(",", " "));
