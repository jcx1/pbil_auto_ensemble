...
public class RunAutoWeka2 {
	...
	private static String output_name = "AutoWeka_Exp1-2";		// nome do arquivo csv	
	private static int seed = 123;								// default = 123
	...
	private static void run(File output, String base, int seed, int mTime, int mem, int nBest, int pRuns) throws FileNotFoundException {
		...
		try {
			...
			for (int fold = 0; fold < numFoldsEvaluate; fold++) {
				...
				numberInternalRuns++;
				System.out.println("Number of Internal Runs: " + numberInternalRuns + " - Error => " + mainError);
			}
