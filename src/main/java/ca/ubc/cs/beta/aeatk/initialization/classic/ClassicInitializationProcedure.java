...
public class ClassicInitializationProcedure implements InitializationProcedure {
	...
	public ClassicInitializationProcedure(ThreadSafeRunHistory runHistory, ParameterConfiguration initialIncumbent, TargetAlgorithmEvaluator tae, 
			ClassicInitializationProcedureOptions opts, InstanceSeedGenerator insc, List<ProblemInstance> instances,  int maxIncumbentRuns , 
			TerminationCondition termCond, double cutoffTime, SeedableRandomPool pool, boolean deterministicInstanceOrdering, AlgorithmExecutionConfiguration execConfig){
	...
	public void run() {
		...
		for(int i=0; i <N; i++){
			...
			try { 
				// JC
				// chama o metodo List<AlgorithmRunResult> evaluateRun(AlgorithmRunConfiguration runConfig)
	...
	protected List<AlgorithmRunResult> evaluateRun(List<AlgorithmRunConfiguration> runConfigs){
		...
		// chama o AlgorithmRunResult
		// chama o TargetAlgorithmEvaluator que eh uma Interface
		// que sera implemnatado por AbstractAlgorithmEvaluator que chama o CommandLineAlgorithmRun
