...
public class CommandLineAlgorithmRun implements Callable<AlgorithmRunResult>{
	...
	// This variable is public only for unit test purposes, this is not guaranteed to be the actual environment variable of child processes
	...
	// If after restoring serialization you need something from this object, you should save it as a separate field. (this seems unlikely) 
	 ...
	 * @param executionIDs 
	 */ 
	public CommandLineAlgorithmRun( AlgorithmRunConfiguration runConfig, TargetAlgorithmEvaluatorRunObserver obs, KillHandler handler, 
			CommandLineTargetAlgorithmEvaluatorOptions options, BlockingQueue<Integer> executionIDs){
		//super( runConfig);
	...
	public synchronized AlgorithmRunResult call() {
		...
		try {
			...
			try {
						...
						while(true)	{
							try {
				               ...
				               currentRuntime.set(runtime);
								...
								// Don't log this since this socket exception is what we expect since most of the time we will be blocked on the socket
				...
				// roda o processo - ver metodo runProcess() - linha 603
						...
						try {
							try { 
								try (BufferedReader procIn = new BufferedReader(new InputStreamReader(innerProcess.getErrorStream()))){
									...
									// JC aparentemente a String lOutra = procIn.readLine(); eh nula
									// System.out.println("procIn.: " + lOutra);
									do{
										String line;
										boolean read = false;
										while(procIn.ready()){										
				...
				try {
					...
					if(jvmShutdownDetected.get()){ //Possible that this run started after the shutdown call was flagged, but before we put it in the map.
						killProcess(proc);
					}
					try	{
			...
			log.error( "The following algorithm call failed: cd \"{}\" " + COMMAND_SEPERATOR + "  {} ", 
					new File(runConfig.getAlgorithmExecutionConfiguration().getAlgorithmExecutionDirectory()).getAbsolutePath(), getTargetAlgorithmExecutionCommandAsString( runConfig));

			throw new TargetAlgorithmAbortException(e1);
		}
	} // AlgorithmRunResult call()
	
		
	/**
	 * Processes all the output of the target algorithm - Takes a line from the input and tries to parse it 
...
outerloop:		
	...
	private  Process runProcess(int port, Integer token) throws IOException	{
		...
		// JC
		// execCmdArray -> pega todos os comandos que serao executados
		// incluindo autoweka.smac.SMACWrapper
		...
		//System.out.println("");
		// JC - envpList pega variaveis do sistema
		//for (String s2 : envpList) {
		...
		// JC - inicia processo de treinamento dos classificadores
