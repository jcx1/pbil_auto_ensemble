...
public class SMACBuilder {
	...
	public AbstractAlgorithmFramework getAutomaticConfigurator(AlgorithmExecutionConfiguration execConfig, InstanceListWithSeeds trainingILWS, SMACOptions options,Map<String, AbstractOptions> taeOptions, String outputDir, SeedableRandomPool pool, TargetAlgorithmEvaluator oTAE, RunHistory oRHModel) {	
		...
		// JC verifica qual a opcao que sera usada pelo RandomForest (RunTime ou Quality)
		...
		// JC - chama TargetAlgorithmEvaluatorOptions.java
		...
		// JC
		// if (initialIncumbent.equals(configSpace.getDefaultConfiguration())) System.out.println("Sim igual");
		...
		// JC - chama ClassicInitializationProcedure.java
		...
		switch(options.execMode){
			case ROAR:
				smac = new AbstractAlgorithmFramework(options,execConfig, instances,acTae,sf, configSpace, instanceSeedGen, 
						initialIncumbent, initialChallengers, eventManager, rh, pool, termCond, configTracker, initProc,cpuTime);
				break;
			// JC - chama a configuracao do SMAC	
			case SMAC:
				smac = new SequentialModelBasedAlgorithmConfiguration(options, execConfig, instances, acTae, options.expFunc.getFunction(),sf, configSpace, instanceSeedGen, 
						initialIncumbent, initialChallengers, eventManager, rh,pool, termCond, configTracker, initProc, rhModel, cpuTime);
				break;
			case PSEL:
