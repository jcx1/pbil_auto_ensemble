	...
	*/
	...
	public static int oldMain(String[] args) {
		...
		try {
			...
			// cria um AbstractAlgorithmFramework - smac
			...
			if(options.doValidation){
				...
				try {
					...
					performance  = (new Validator()).simpleValidate(testInstances,options.validationOptions,options.scenarioConfig.algoExecOptions.cutoffTime, testInstanceSeedGen, 
							validatingTae, outputDir, options.scenarioConfig.getRunObjective(), options.scenarioConfig.getIntraInstanceObjective(), options.scenarioConfig.interInstanceObj, trajFile, true, coreHint, execConfig);
	...
	private static SMACOptions parseCLIOptions(String[] args) throws ParameterException, IOException{
		...
		try {
			...
			System.out.println("Verificando .... dentro -> SMACExecutor ....");
			...
			log.info(CommonMarkers.SKIP_CONSOLE_PRINTING,"==========Enviroment Variables===========");
			System.out.println("Comentada linha ...: 442 - dentro -> SMACExecutor");
			...
			log.info(CommonMarkers.SKIP_CONSOLE_PRINTING,"==========System Properties==============");
			System.out.println("Comentada linha ...: 461 - dentro -> SMACExecutor");
			
			// chama ca.ubc.cs.beta.aeatk.misc.jcommander.JCommanderHelper.java 
