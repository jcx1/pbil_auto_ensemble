...
public class AutoWEKAClassifier extends AbstractClassifier implements AdditionalMeasureProducer {
    ...
    // JC alterei o tempo limite para 1 - era 15
    ...
    // JC - especificacao da metrica a ser usada
    static final Metric DEFAULT_METRIC = Metric.errorRate;
    // static final Metric DEFAULT_METRIC = Metric.meanAbsoluteError;
    ...
    public String getBestConfiguration(){
    	return classifierClass + " " + Utils.arrayToString(classifierArgs);
    }
    ...
    */
    public void buildClassifier(Instances is) throws Exception {
        ...
        for(int i = 0; i < parallelRuns; i++) {
            ...
            //System.out.println("autoweka" + File.separator);
            
            Experiment exp = new Experiment();
            exp.name = expName;

            exp.resultMetric = metric.toString();
            //System.out.println("metric => " + exp.resultMetric);
            ...
            // JC seta se vai usar attribute Selection 
            ...
            //Make the thing
            // JC - chama aqui o ExperimentConstructor -> que vai chamar o SMACExperimentConstructor -> responsavel por achar as propriedades autoweka.smac.SMACExperimentConstructor.properties
            // JC - esse arquivo indica qual o *.bat sera executato -> smac.bat - usado para setar o path e executar a classe "ca.ubc.cs.beta.smac.executors.SMACExecutor"
            ExperimentConstructor.buildSingle("autoweka.smac.SMACExperimentConstructor", exp, args);

            //System.out.println("nBestConfigs =>" +  nBestConfigs);
            if(nBestConfigs > 1) {
            ...
            workers[i] = new Thread(new Runnable() {
                public void run() {
                    ...
                    try {
                    	...
                    	// JC - chama => autoweka.tools.ExperimentRunner
                    	// System.out.println("AutoWekaClassifier.java - ClassPath Absoluto.: " + autoweka.Util.getAbsoluteClasspath());
                        ...
                        while((line = reader.readLine()) != null) {
                            ...
                            if(line.matches(".*DEBUG.*") || line.matches(".*Variance is less than.*")) {
                                ...
                                if(line.matches(".*ClassifierRunner - weka.classifiers.*")) {
                                    ...
                                    totalTried++;
                                    System.out.println("Config. " + totalTried + ": " + line.substring(211));
                                    //System.out.println("Configuração " + totalTried + ": " + line);
        ...
        // mostrar a melhor configuração ....
        String args = "[";
        System.out.println("----------------------------------------------------------------------------------------------------------");
        //System.out.println("Melhor configuração: " + classifierClass + ";[" + Utils.arrayToString(classifierArgs) + "]");
        for (String s : classifierArgs){
        	args += s + ", ";
        }
        args = args.substring(0, args.length() - 2);
        args += "]"; 
        System.out.println("Melhor config.: " + classifierClass + ";" + args);
        System.out.println("----------------------------------------------------------------------------------------------------------");
